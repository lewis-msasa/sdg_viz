...
import { GDPData, flagImages, yearGDPDescriptions as yearDescriptions } from '../data/countryData';
...
const AllCountriesAnimatedGDPLineChart = () => {
  ...
  const countries = data.map(d => d.shortName);
...
export const CountryAnimatedGDPLineChart = ({countryName}) => {
  // Refs
  const svgRef = useRef(null);
  const gRef = useRef(null);
  const pathsRef = useRef([]);
  ...
  const data  = GDPData.filter(d => d.country == countryName);
  console.log(data[0])
  const years = data[0].values.map(d => d.year);
  console.log(years)
  const lastYear = years[years.length-1];
  ...
  useEffect(() => {
    ...
    gRef.current.append("text")
      ...
      .attr("y", -30)
      .attr("text-anchor", "middle")
      .text(`GDP Growth for ${countryName} (2010-2023)`);
